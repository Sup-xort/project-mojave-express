...
function currentSettings() {
  return {
    ...
    // 아래 셋은 .env에서만 바꾼다. 화면에는 읽기 전용으로만 보여준다.
...
// 필드별 부분 업데이트. 보낸 항목만 바꾸고 나머지는 건드리지 않는다.
router.put('/settings', (req, res, next) => {
  const body = req.body || {};

  if ('couponTtlDays' in body) {
    const raw = body.couponTtlDays;
  ...
  if ('qrInstantIssue' in body) {
    settingsService.setQrInstantIssue(body.qrInstantIssue === true);
  }
